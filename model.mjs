...
export function seed(){
 const symptoms=[2,0,3,1,4,null,2,1,3,2,1,4,2];
 const sleeps=[7.4,8,6.1,7.8,5.6,null,7.1,8.2,6.3,7.2,7.9,5.8,7.4];
 return {version:1, observations:symptoms.flatMap((v,i)=>v===null?[]:[{id:'day-'+i,date:`2026-09-${String(i+1).padStart(2,'0')}`,symptom:'Headache',severity:v,sleep:sleeps[i],note:i===11?'Busy afternoon; rested at home.':'Sample daily check-in',source:'Sample journal'}]),questions:[{id:'q1',text:'What should I keep track of between appointments?'},{id:'q2',text:'Could we discuss the pattern in my headache journal?'},{id:'q3',text:'Which changes should prompt me to contact the clinic?'}],records:[{id:'r1',title:'Follow-up consultation',type:'Visit note',date:'2026-08-28',provider:'Sample primary care clinic',body:'DEMONSTRATION RECORD — FICTIONAL PATIENT\nPatient: Maya Tan\nDate: 28 August 2026\nHistory: Recurrent headaches reported by patient.\nFollow-up: Bring symptom journal to next appointment.\nDocumented medication: No medication details supplied in this sample.\nThis sample contains no treatment instructions.'},{id:'r2',title:'Health screening results',type:'Lab report',date:'2026-08-12',provider:'Sample screening centre',body:'DEMONSTRATION RECORD — FICTIONAL PATIENT\nPatient: Maya Tan\nDate: 12 August 2026\nHaemoglobin: 13.1 g/dL\nLaboratory reference range: 12.0–15.5 g/dL\nDo not use synthetic values for health decisions.'}],messages:[],reminders:[{id:'rem1',text:'Evening check-in',time:'20:30',active:true},{id:'rem2',text:'Prepare appointment questions',time:'18:00',active:true}],family:{active:true,records:false,journal:true,visits:true},medications:[{name:'Medication list',detail:'Add only medication information provided by your clinician.',reported:false}],pending:null};
}
export function addObservation(state, entry){return {...state,observations:[...state.observations,{id:entry.id||'obs-'+Date.now(),source:'Conversation',...entry}]};}
export function editObservation(state,id,patch){return {...state,observations:state.observations.map(o=>o.id===id?{...o,...patch}:o)};}
