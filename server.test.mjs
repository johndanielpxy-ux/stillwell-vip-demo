...
test('journey module served without opening arbitrary files',async t=>{const base=await withServer(t,{},mock(reply));assert.equal((await fetch(base+'/journey.mjs')).status,200);const font=await fetch(base+'/assets/instrument-sans.ttf');assert.equal(font.status,200);assert.equal(font.headers.get('content-type'),'font/ttf');assert.equal((await fetch(base+'/visual.css')).status,200);assert.equal((await fetch(base+'/journey.test.mjs')).status,404);});
